...
from sae_pipeline import autoencoder_training, dataset_sequencing
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--stage",
        choices=["sequence", "train-sae", "analyze", "finalize-analysis", "all"],
        default="all",
    )
    ...
    parser.add_argument("--tokens-path", default=None, help="Prepared token .npy file; bypasses JSONL sequencing")
    parser.add_argument("--attention-mask-path", default=None, help="Prepared attention-mask .npy file")
    parser.add_argument(
        "--require-prepared-sequences",
        action="store_true",
        help="Fail if prepared token/mask files are missing; never tokenize JSONL.",
    )
    ...
    parser.add_argument("--analysis-checkpoint-path", default=None)
    parser.add_argument("--no-analysis-resume", action="store_true")
    parser.add_argument("--analysis-checkpoint-every-chunks", type=int, default=10)
...
def _resolve_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    profile.update({
        ...
        "require_prepared_sequences": args.require_prepared_sequences,
        ...
        "analysis_checkpoint_path": args.analysis_checkpoint_path,
        "no_analysis_resume": args.no_analysis_resume,
        "analysis_checkpoint_every_chunks": args.analysis_checkpoint_every_chunks,
...
def _prepare_sequences(args: argparse.Namespace, tokenizer) -> None:
    if args.tokens_path is not None or args.attention_mask_path is not None:
        missing = [
            str(path)
            for path in (args.tokens_path, args.attention_mask_path)
            if path is None or not Path(path).is_file()
        ]
        if missing:
            raise FileNotFoundError(
                "Prepared sequences were requested, but files are missing: "
                + ", ".join(missing)
            )
        print(
            "Using prepared token sequences directly from: "
            f"{args.tokens_path}; mask: {args.attention_mask_path}"
        )
        return
    if args.require_prepared_sequences:
        raise FileNotFoundError(
            "--require-prepared-sequences requires --tokens-path and "
            "--attention-mask-path."
        )
...
def _analyze_sae(args: argparse.Namespace, tokenizer) -> None:
    import torch
    from sae_pipeline import features_analysis
    ...
    features_analysis.analyze_sae(
        ...
        attention_mask_path=args.attention_mask_path,
        analysis_checkpoint_path=args.analysis_checkpoint_path,
        resume=not args.no_analysis_resume,
        checkpoint_every_chunks=args.analysis_checkpoint_every_chunks,
    )


def _finalize_analysis(args: argparse.Namespace, tokenizer) -> None:
    import torch
    from sae_pipeline import features_analysis
    from sae_pipeline.autoencoder_training import TopKSAE

    if not args.checkpoint_path:
        raise ValueError("--stage finalize-analysis requires --checkpoint-path for the SAE")
    if not args.analysis_checkpoint_path:
        raise ValueError(
            "--stage finalize-analysis requires --analysis-checkpoint-path"
        )
    ...
    sae.load_state_dict(payload["model_state_dict"])
    model = AutoModelForCausalLM.from_pretrained(args.model_name)
    features_analysis.finalize_analysis_checkpoint(
        sae=sae,
        model=model,
        tokenizer=tokenizer,
        path_dir=args.data_path,
...
def main() -> None:
    ...
    if args.stage == "finalize-analysis":
        _finalize_analysis(args, tokenizer)
