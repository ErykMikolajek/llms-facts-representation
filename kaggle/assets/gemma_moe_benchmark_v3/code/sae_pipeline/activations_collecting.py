...
def get_backbone(model: torch.nn.Module) -> torch.nn.Module:
    ...
    # Gemma 3 exposes the decoder-only backbone as ``model.model`` in
    # HuggingFace Transformers.  Keep this adapter here so the existing
    # streaming token/mask path can also be reused by Gemma Scope analysis.
    ...
    raise ValueError(
        ...
        "GPT-Neo ``transformer``, or Gemma/Llama-style ``model.layers`` backbone."
    )
...
def open_token_store(
    ...
    tokens_path = Path(tokens_path) if tokens_path is not None else root / "tokens_seqs_padded.npy"
    mask_path = Path(mask_path) if mask_path is not None else root / "attention_mask.npy"
...
def iter_activation_chunks(
    ...
    store = open_token_store(data_path, seq_length, tokens_path=tokens_path, mask_path=mask_path)
