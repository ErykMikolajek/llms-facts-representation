...
| **Layer analyzed** | Layer **4** (configurable via `LAYER_NUM` in `sae_pipeline/main.py`) |
...
The project implements an end-to-end pipeline orchestrated by `sae_pipeline/main.py`. It consists of **four sequential stages**:

```
Dataset Sequencing → Activation Collection → Autoencoder Training → Feature Analysis
```

### 3.1. Stage 1 — Dataset Sequencing (`sae_pipeline/dataset_sequencing.py`)
...
### 3.2. Stage 2 — Activation Collection (`sae_pipeline/activations_collecting.py`)
...
### 3.3. Stage 3 — Autoencoder Training (`sae_pipeline/autoencoder_training.py`)
...
### 3.4. Stage 4 — Feature Analysis (`sae_pipeline/features_analysis.py`)
...
## 4. Utility Module (`common/utils.py`)
...
├── sae_pipeline/                # Sequencing, activations, SAE training/analysis
├── domain_triage/               # Domain discovery, stability and selection
├── domain_mapping/              # Domain data, SAE–MLP mapping and pruning
├── moe/                         # Router, assembly, freeze and runtime bundle
├── evaluation/                  # Holdout PPL and independent benchmark
├── kaggle/                      # CLI, notebooks, builders and packaged assets
├── tests/                       # Unit and regression tests
├── reports/                     # Research reports
├── results/                     # Imported evaluation outputs
├── documentation/               # MkDocs sources and architecture documents
├── common/                      # Shared device and reproducibility helpers
├── README.md
├── requirements.txt
├── constraints.txt
│
...
| `7732115` | Did some experiments |

The table above records early repository history only; it is not intended to
identify the current branch or working-tree state.
...
|-----------|-------|----------|
| Model | `roneneldan/TinyStories-1M` | `sae_pipeline/main.py` |
| Tokenizer | `EleutherAI/gpt-neo-125M` | `sae_pipeline/main.py` |
| Sequence length | 256 | `sae_pipeline/main.py` |
| Layer number | 4 | `sae_pipeline/main.py` |
| Dataset fraction | 1% | `sae_pipeline/main.py` |
| LLM batch size | 4 | `sae_pipeline/main.py` |
| SAE input dim (`d_model`) | 64 | `sae_pipeline/main.py` |
| SAE expansion factor | 64 (→ 4096 features) | `sae_pipeline/main.py` |
| Top-K sparsity | 8 | `sae_pipeline/main.py` |
| SAE batch size | 256 | `sae_pipeline/main.py` |
| SAE epochs | 5 | `sae_pipeline/main.py` |
| Learning rate | 1e-3 | `sae_pipeline/main.py` |
| LR scheduler | Cosine annealing (min: 1e-5) | `sae_pipeline/autoencoder_training.py` |
| Activation threshold | 0.03 | `sae_pipeline/features_analysis.py` |

## 8a. Active Pythia SAE Profile

The active `local-50gb` profile in `sae_pipeline/main.py` uses `EleutherAI/pythia-160m`,
