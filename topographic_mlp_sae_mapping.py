...
def pearson_by_neuron(mlp_activations: np.ndarray, cluster_activations: np.ndarray) -> np.ndarray:
    import numpy as np

    x = mlp_activations.astype(np.float64, copy=False)
    y = cluster_activations.astype(np.float64, copy=False)
    ...
    denominator = np.sqrt((x_centered**2).sum(axis=0) * np.sum(y_centered**2))
    return np.divide(
        numerator,
        denominator,
